import os, pgzrun, sys, pygame, random
...
def setup_game():
    ...
    game_over = False
    player_won = False
    player = Actor("fox", anchor=("left", "top"))
    keys_to_collect = []
    guards = []
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            ...
            if square == "P":
                ...
                key = Actor("burger", anchor=("left", "top"), pos=screen_coords(x, y))
                keys_to_collect.append(key)
            elif square == "G":
                guard = Actor("bear", anchor=("left", "top"), \
...
def draw_background():
    ...
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            ...
            if n < 5:
                screen.blit("grass", screen_coords(x,y))
            elif n < 10:
                screen.blit("grass2", screen_coords(x,y))
...
def draw_scenery():
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            square = MAP[y][x]
            if square == "W":
                screen.blit("forest", screen_coords(x,y))
            elif square == "D":
                screen.blit("picnic", screen_coords(x,y))
...
def draw_game_over():
    ...
    screen.draw.text("Press SPACE to play again. Press ESCAPE to close.", midtop=(WIDTH / 2, \
...
def on_key_up(key):
    if key == keys.SPACE and game_over:
        setup_game()
    if key == keys.ESCAPE and game_over:
        exit_game()
